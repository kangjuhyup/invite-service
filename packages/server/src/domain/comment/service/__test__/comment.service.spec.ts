...
describe('CommentService', () => {
    ...
    describe('addComment', () => {
        it('새로운 댓글을 추가해야 함', async () => {
            ...
            when(mockLetterRepository.insertComment(anything())).thenResolve(insertResult);
    ...
    describe('selectComments', () => {
        it('특정 초대장의 댓글 목록을 조회해야 함', async () => {
            ...
            verify(mockLetterRepository.selectComments(deepEqual({letterId: 1}))).once();
        });
    });
    ...
    describe('deleteComment', () => {
        it('특정 댓글을 삭제해야 함', async () => {
            // given
            const commentId = 1;
            when(mockLetterRepository.deleteComment(deepEqual({letterCommentId: commentId}))).thenResolve();

            // when
            await service.deleteComment(commentId);

            // then
            verify(mockLetterRepository.deleteComment(deepEqual({letterCommentId: commentId}))).once();
        });
    });
    ...
    describe('deleteCommentFromLetter', () => {
        it('초대장의 모든 댓글을 삭제해야 함', async () => {
            // given
            const letterId = 1;
            when(mockLetterRepository.deleteCommentFromLetter(deepEqual({letterId}))).thenResolve();

            // when
            await service.deleteCommentFromLetter(letterId);

            // then
            verify(mockLetterRepository.deleteCommentFromLetter(deepEqual({letterId}))).once();
        });
    });
    ...
    describe('checkCommentAuthor', () => {
        it('올바른 비밀번호로 인증 성공해야 함', async () => {
            ...
            // when & then
            await expect(service.checkCommentAuthor(commentId, password)).resolves.not.toThrow();
            verify(mockLetterRepository.selectComment(deepEqual({letterCommentId: commentId}))).once();
        });

        it('댓글이 존재하지 않으면 NotFoundException을 던져야 함', async () => {
            // given
            const commentId = 1;
            when(mockLetterRepository.selectComment(deepEqual({letterCommentId: commentId}))).thenResolve(null);

            // when & then
            await expect(service.checkCommentAuthor(commentId, 'anyPassword'))
                .rejects
        ...
        it('잘못된 비밀번호로 UnauthorizedException을 던져야 함', async () => {
            ...
            // when & then
            await expect(service.checkCommentAuthor(commentId, wrongPassword))
                .rejects
                .toThrow(new UnauthorizedException('잘못된 비밀번호 입니다.'));
        });
    });
});
