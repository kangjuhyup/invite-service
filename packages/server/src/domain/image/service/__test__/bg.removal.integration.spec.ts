...
describe('BackgroundRemovalService (Integration)', () => {
    ...
    const TEST_ASSETS_DIR = path.join(PROJECT_ROOT,'src','domain','image','service','__test__');
    ...
    describe('실제 배경 제거 테스트', () => {
        ...
        beforeAll(async () => {
            ...
            try {
                ...
                // 테스트 이미지 다운로드
                const response = await fetch('https://raw.githubusercontent.com/imgly/background-removal-js/main/example/example.jpg');
